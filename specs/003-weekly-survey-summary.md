...
- All pre-existing service/controller/e2e tests for `getSurveySummary` (written ahead of this implementation) pass unchanged; no test edits were needed to make the implementation fit. Client-side work (types, `survey-summary-view.tsx`) is not yet done — API-only pass.
